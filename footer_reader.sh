...
echo "Reading traces with timestamp $ts from directory $dir." 
for tracefile in `ls $dir/dumpi-$ts-*.bin`; do
	# echo "Examining $tracefile :" 
	for i in ${!tracedCalls[@]}; do
		...
		# echo "${tracedCalls[$i]} occurs in $tracefile ${callLineArray[2]} times."
...
results="$4"
echo "Reporting results in file $4."
for i in ${!tracedCalls[@]}; do
	echo "${tracedCalls[$i]} : ${tracedCallsNums[$i]}" >> $results
done
